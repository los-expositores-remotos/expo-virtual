...
class FeedServiceTest {
    ...
    fun creamosUnProveedor() {
        ...
        mongo.database.getCollection("pruebaVV").insertOne(document)
    }

    @Test
    fun cantidadDeProveedores() {
        //println(mongo.database.countItemsFromCollection("pruebaVV"))
    }

    @Test
    fun buscarPoveedoresConUnCriterio() {
        //var documents = mongo.database.findEq<String, String>("lastname", "Suarez", "pruebaVV")
        if (documents != null) {
            for (d in documents) {
                println(d)
            }
